...
/*   Updated: 2020/09/08 19:38:46 by matascon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_app(t_env *env)
{
	...
	//adjust_screen(env);
	if (!(env->win_ptr = mlx_new_window(env->mlx_ptr, env->width_screen, \
	env->height_screen, "Cub3D")))
		ft_exit(env, ERROR_15);
	ft_init_textures(env);
	ft_render_frame(env);
	mlx_hook(env->win_ptr, 2, 1, &ft_key_press, env);
	mlx_hook(env->win_ptr, 3, 2, &ft_key_release, env);
	mlx_hook(env->win_ptr, 17, 1L << 17, &ft_key_exit, env);
